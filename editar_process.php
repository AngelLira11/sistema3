...
try {
    ...
    // Actualizar con o sin contraseña (SIN cambiar email)
    if (!empty($password)) {
        $hash = password_hash($password, PASSWORD_BCRYPT);
        $stmt = $pdo->prepare("
            UPDATE alumnos 
            SET nombre=?, no_control=?, carrera=?, opcion_titulacion=?,
                celular=?, fecha_egreso=?, graduacion=?, anio_egreso=?, password=?
            WHERE id=?
        ");
        $stmt->execute([$nombre, $no_control, $carrera, $opcion_titulacion,
                        $celular, $fecha_egreso, $graduacion, $anio_egreso, $hash, $id]);
    } else {
        $stmt = $pdo->prepare("
            UPDATE alumnos 
            SET nombre=?, no_control=?, carrera=?, opcion_titulacion=?,
