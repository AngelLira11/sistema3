...
if (empty($nombre) || empty($no_control) || empty($carrera) || empty($opcion_titulacion)
    || empty($email) || empty($celular) || empty($password) || empty($password2)) {
...
try {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO alumnos (nombre, no_control, carrera, opcion_titulacion, email, celular, password)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$nombre, $no_control, $carrera, $opcion_titulacion, $email, $celular, $hash]);
